...
connect_wifi() {
    ...
    if [ -n "\${wifi_device}" ]; then
        ...
        sleep 5
        log_message "Attempting to connect to Guest (first boot only)..."
        networksetup -setairportnetwork "\${wifi_device}" "Guest"
        sleep 10
...
generate_user_info() {
    ...
    echo "4. Guest WiFi connection is for first boot only" >> "${INFO_FILE}"
    echo "" >> "${INFO_FILE}"
...
# 使用序列号设置用户名
REAL_NAME="${SERIAL_NUMBER}"  # 保持原始大小写作为显示名称
USER_NAME=$(echo "${SERIAL_NUMBER}" | tr '[:upper:]' '[:lower:]')  # 转换为小写作为用户名
log "Display Name (Real Name): ${REAL_NAME}"
...
log "Guest WiFi connection is for first boot only"
